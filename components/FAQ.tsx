      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-24 items-start">
        ...
        <div className="reveal lg:col-span-7 bg-[#E5E5E5] rounded-[20px] p-[7px] flex flex-col gap-3 transition-none" style={{ transitionDelay: '500ms' }}>
          {faqs.map((faq, idx) => (
            <div 
              key={idx} 
              className={`bg-[#F0F0F0] rounded-[16px] overflow-hidden transition-all duration-700 opacity-0 translate-y-8 [.reveal-visible_&]:opacity-100 [.reveal-visible_&]:translate-y-0`}
              style={{ 
            ...
            >
              ...
              >
                <h3 className="text-[18px] font-black tracking-tight text-[#111111] group-hover:text-[#203C71] transition-colors duration-300">
                  {faq.question}
                </h3>
                ...
                <div className="relative w-11 h-11 bg-[#E5E5E5] rounded-full flex items-center justify-center shrink-0 group-hover:bg-[#dcdcdc] transition-colors duration-300">
                  ...
                  <div className={`absolute w-5 h-[2.5px] rounded-full transition-all duration-500 ${openIndex === idx ? 'bg-[#EF7F17]' : 'bg-[#203C71]'}`}></div>
                  
                  {/* Vertical Bar (Part of the Plus) */}
                  <div className={`absolute w-[2.5px] h-5 bg-[#203C71] rounded-full transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] ${openIndex === idx ? 'scale-y-0 opacity-0 rotate-90' : 'scale-y-100 opacity-100 rotate-0'}`}></div>
                </div>
              </button>
