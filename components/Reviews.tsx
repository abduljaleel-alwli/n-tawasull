...
const initialTestimonials = [
  {
    rating: 4.9,
    quote: "مبادرون، دقيقون، ومن السهل العمل معهم — لا يحتاجون لتوجيه مستمر، مجرد تعاون سلس من البداية للنهاية.",
    name: "خالد سعيد",
    role: "مدير التسويق",
  ...
  {
    rating: 5.0,
    quote: "شعرنا وكأنهم جزء من فريقنا؛ كان التواصل واضحاً، والتعديلات تم تنفيذها بشكل مثالي من المرة الأولى.",
    name: "ليلى حسن",
    role: "رئيسة المنتج",
...
const Reviews: React.FC = () => {
  ...
  const badge = getSetting('reviews.badge', 'التقييمات');
  const title = getSetting('reviews.title', 'قصص نجاح.');
  const description = getSetting('reviews.description', 'اكتشف كيف يساعد اشتراك التسويق الخاص بنا العلامات التجارية المبتكرة على النمو بشكل أذكى وأسرع.');
  
  const summary = getSetting('reviews.summary', {
    rating_text: "4.9/5",
    note: "لقد سلمنا +100 مشروع حققت نتائج حقيقية لعملائنا.",
    avatars: ["https://i.pravatar.cc/150?u=rev4","https://i.pravatar.cc/150?u=rev3","https://i.pravatar.cc/150?u=rev2","https://i.pravatar.cc/150?u=rev1"],
    trust_title: "موثوق به من +100 شركة",
    trust_subtitle: "لقد حققوا أهدافهم — أنت التالي."
  });

  const cta = getSetting('reviews.cta', {
    label: "اترك تقييماً",
    url: "https://n-tawasull.sa/"
  });

  const testimonials = getSetting('reviews.items', initialTestimonials);
      ...
      <div className="reveal grid grid-cols-1 lg:grid-cols-12 gap-6 transition-none" style={{ transitionDelay: '500ms' }}>
        ...
        >
          <div className="flex justify-between items-start">
            <div className="text-7xl font-black tracking-tighter">{summary.rating_text}</div>
            <div className="max-w-[150px] text-right">
              <p className="text-sm font-bold leading-tight text-white/70">{summary.note}</p>
            </div>
          </div>
          ...
          <div className="space-y-6">
            <div className="flex flex-col items-start gap-4">
              <div className="flex -space-x-3 space-x-reverse">
                {Array.isArray(summary.avatars) && summary.avatars.map((avatar: string, i: number) => (
              ...
              <div className="space-y-1 text-right">
                ...
                <h4 className="text-lg font-black mt-2 text-white">{summary.trust_title}</h4>
                <p className="text-[10px] font-black text-white/50 tracking-wider uppercase">{summary.trust_subtitle}</p>
              </div>
            </div>

            <a href={cta.url} target="_blank" rel="noopener noreferrer" className="group relative block w-full bg-background text-primary py-5 rounded-full font-black text-lg overflow-hidden transition-all duration-500 hover:bg-white active:scale-95 shadow-xl" data-cursor-text="شارك رأيك">
              <div className="relative h-7 overflow-hidden pointer-events-none">
                <div className="flex flex-col transition-transform duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] group-hover:-translate-y-1/2">
                  <span className="h-7 text-lg font-black leading-7 whitespace-nowrap flex items-center justify-center">{cta.label}</span>
                  <span className="h-7 text-lg font-black leading-7 whitespace-nowrap flex items-center justify-center">{cta.label}</span>
                </div>
              </div>
            </a>
        ...
        <div className="lg:col-span-7 grid grid-cols-1 md:grid-cols-2 gap-6 transition-none">
          {Array.isArray(testimonials) && testimonials.map((item: any, idx: number) => (
            <div 
              key={idx} 
            ...
            >
              ...
              <div className="px-6 py-4 flex items-center gap-4 text-right mt-auto">
                ...
                <div className="flex flex-col">
                  <span className="text-sm font-black text-[#111111]">{item.name}</span>
